...
You can start by copying your type-annotated syntax definition and type checker from Lab 2.
...
For non-`void`-returning functions other than `main` you are allowed to assume that returns are always properly placed by the programmer who wrote the input source code.
However, for `void`-returning functions and `main` you must consider the possibility that no explicit return statement was included in the program source code.
...
These stubs are found in the git branches `start-haskell` and `start-java`.
The default is `start-haskell`, so the `main` branch points there.

To switch to `start-java`, you need to switch your main branch and force-push it:
```console
$ git switch -C main origin/start-java
$ git push --force origin main
```
(Observe that `switch -C` and `push --force` are destructive `git` commands, so only use them in the very beginning lest you might lose work already done!)

You find there the grammar `CMM.cf`, stubs for type-annotated syntax, type checker and compiler, the testsuite, and a suitable `Makefile`.
There is also a `README-haskell.md` or `README-java.md` with further information.
...
For help with building the Jasmin file, refer to the [Jasmin instructions](http://jasmin.sourceforge.net/instructions.html) and [Java bytecode instruction listings](https://en.wikipedia.org/wiki/List_of_Java_bytecode_instructions).
...
2. Make sure your solution has the following structure:
   - The root directory contains the sources of your solution plus a `Makefile`.
   - After `make` is invoked, there should be a runnable program `lab3` (on Windows: `lab3.exe`).
   - This program takes a filepath as command-line parameter and type-checks and compiles this file, printing type errors to the standard output.

   See the example [solution templates](#solution-templates).
...
_(C) 2022-24 Andreas Abel and the PLT Team.
